...
static void assert_nocalibration_overlap(const block &newBlock, uint8_t idxCurrBlock, SensorCalibrationTable table) {
    ...
    char msg[64];
    sprintf(msg, "EEPROM storage: entity %" PRIu16 " overlaps calibration CRC %" PRIu16, idxCurrBlock, table);
...
static void print_eeprom_layout(void) {
    ...
    #define GET_VARIABLE_NAME(Variable) (#Variable)
    char msg[128];
    sprintf(msg, "Calib. CRC, %d, %s, CRC, %" PRIu16 ", %zu", (int)CoolantSensor, GET_VARIABLE_NAME(CoolantSensor), getSensorCalibrationCrcAddress(CoolantSensor), sizeof(uint32_t));
    UnityPrint(msg); UNITY_PRINT_EOL();
    sprintf(msg, "Calib. CRC, %d, %s, CRC, %" PRIu16 ", %zu", (int)IntakeAirTempSensor, GET_VARIABLE_NAME(IntakeAirTempSensor), getSensorCalibrationCrcAddress(IntakeAirTempSensor), sizeof(uint32_t));
    UnityPrint(msg); UNITY_PRINT_EOL();
    sprintf(msg, "Calib. CRC, %d, %s, CRC, %" PRIu16 ", %zu", (int)O2Sensor, GET_VARIABLE_NAME(O2Sensor), getSensorCalibrationCrcAddress(O2Sensor), sizeof(uint32_t));
    UnityPrint(msg); UNITY_PRINT_EOL();
