...
void test_findBin_clamplower(void)
{
    ...
    uint8_t result = find_bin(find_u8, test_data_u8, test_data_u8_start, test_data_u8_end, 2U).maxIdx;
    TEST_ASSERT_EQUAL_UINT8(1, result);

    int16_t find_s16 = test_data_s16[test_data_s16_start]-5;
    result = find_bin(find_s16, test_data_s16, test_data_s16_start, test_data_s16_end, 2U).maxIdx;
    TEST_ASSERT_EQUAL_UINT8(1, result);
}
...
void test_findBin_clamplower_reverse(void)
{
    ...
    uint8_t result = find_bin(find_u8, test_data_u8_r, test_data_u8_r_start, test_data_u8_r_end, 2U).maxIdx;
    TEST_ASSERT_EQUAL_UINT8(test_data_u8_r_start-1, result);

    int16_t find_s16 = test_data_s16_r[test_data_s16_r_start]-5;
    result = find_bin(find_s16, test_data_s16_r, test_data_s16_r_start, test_data_s16_r_end,  2U).maxIdx;
...
void test_findBin_bin_edges(void)
{
    ...
    for (uint8_t loop=test_data_u8_start+1; loop!=test_data_u8_end+1; ++loop) 
    {
        ...
        result = find_bin(find_u8, test_data_u8, test_data_u8_start, test_data_u8_end, 2U).maxIdx;
...
void test_findBin_bin_edges_reverse(void)
{
    ...
    for (uint8_t loop=test_data_u8_r_start-1; loop!=test_data_u8_r_end; --loop) 
    {
        ...
        result = find_bin(find_u8, test_data_u8_r, test_data_u8_r_start, test_data_u8_r_end, 2U).maxIdx;
        TEST_ASSERT_EQUAL_UINT8(loop, result);
    }    
}
...
void test_findBin_bin_midpoints(void)
{
    for (uint8_t loop=0; loop!=_countof(test_data_u8)-2; ++loop) 
    {
        ...
        uint8_t result = find_bin(midPoint, test_data_u8, test_data_u8_start, test_data_u8_end, 2U).maxIdx;
