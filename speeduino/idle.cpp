...
void initialiseIdle(bool forcehoming)
{
  ...
  switch(configPage6.iacAlgorithm)
  {
    ...
    case IAC_ALGORITHM_ONOFF:
      ...
      if ((toStorageTemperature(currentStatus.coolant)) < configPage6.iacFastTemp)
      {
        IDLE_PIN_HIGH();
        idleOn = true;
      }
    ...
    case IAC_ALGORITHM_PWM_CL:
      ...
      idlePID.SetTunings(configPage6.idleKP, configPage6.idleKI, configPage6.idleKD);
      idlePID.SetMode(AUTOMATIC); //Turn PID on
      idle_pid_target_value = table2D_getValue(&iacCrankDutyTable, toStorageTemperature(currentStatus.coolant));
      idlePID.Initialize();
      idleCounter = 0;
...
void idleControl(void)
{
  ...
  switch(configPage6.iacAlgorithm)
  {
    ...
    case IAC_ALGORITHM_ONOFF:      //Case 1 is on/off idle control
      if ( (toStorageTemperature(currentStatus.coolant)) < configPage6.iacFastTemp) //All temps are offset by 40 degrees
      {
        IDLE_PIN_HIGH();
        idleOn = true;
    ...
    case IAC_ALGORITHM_PWM_OL:      //Case 2 is PWM open loop
      ...
      if( BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK) )
      {
        ...
        currentStatus.idleLoad = table2D_getValue(&iacCrankDutyTable, toStorageTemperature(currentStatus.coolant)); //All temps are offset by 40 degrees
        idleTaper = 0;
      }
      ...
      else
      {
        if ( idleTaper < configPage2.idleTaperTime )
        {
          ...
          table2D_getValue(&iacCrankDutyTable, toStorageTemperature(currentStatus.coolant)),\
          table2D_getValue(&iacPWMTable, toStorageTemperature(currentStatus.coolant)));
          if( BIT_CHECK(LOOP_TIMER, BIT_TIMER_10HZ) ) { idleTaper++; }
        }
        else
        {
          //Standard running
          currentStatus.idleLoad = table2D_getValue(&iacPWMTable, toStorageTemperature(currentStatus.coolant)); //All temps are offset by 40 degrees
        }
    ...
    case IAC_ALGORITHM_PWM_OLCL: //case 6 is PWM Open Loop table as feedforward term plus closed loop. 
      ...
      else
      {
        ...
        FeedForwardTerm = percentage(table2D_getValue(&iacPWMTable, toStorageTemperature(currentStatus.coolant)), idle_pwm_max_count<<2); //All temps are offset by 40 degrees
    ...
    case IAC_ALGORITHM_STEP_OL:    //Case 4 is open loop stepper control
      ...
      if( (checkForStepping() == false) && (isStepperHomed() == true) ) //Check that homing is complete and that there's not currently a step already taking place. MUST BE IN THIS ORDER!
      {
        ...
        {
          //Currently cranking. Use the cranking table
          idleStepper.targetIdleStep = table2D_getValue(&iacCrankStepsTable, (toStorageTemperature(currentStatus.coolant))) * 3; //All temps are offset by 40 degrees. Step counts are divided by 3 in TS. Multiply back out here
        ...
        else
        {
          ...
          if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_10HZ) && (currentStatus.RPM > 0))
          {
            if ( idleTaper < configPage2.idleTaperTime )
            {
              ...
              table2D_getValue(&iacCrankStepsTable, (toStorageTemperature(currentStatus.coolant))) * 3,\
              table2D_getValue(&iacStepTable, (toStorageTemperature(currentStatus.coolant))) * 3);
              if( BIT_CHECK(LOOP_TIMER, BIT_TIMER_10HZ) ) { idleTaper++; }
            }
            else
            {
              //Standard running
              idleStepper.targetIdleStep = table2D_getValue(&iacStepTable, (toStorageTemperature(currentStatus.coolant))) * 3; //All temps are offset by 40 degrees. Step counts are divided by 3 in TS. Multiply back out here
            }
            if(currentStatus.idleUpActive == true) { idleStepper.targetIdleStep += configPage2.idleUpAdder; } //Add Idle Up amount if active
      ...
      if( (checkForStepping() == false) && (isStepperHomed() == true) ) //Check that homing is complete and that there's not currently a step already taking place. MUST BE IN THIS ORDER!
      {
        ...
        else 
        {
          if( BIT_CHECK(LOOP_TIMER, BIT_TIMER_10HZ) )
          {
            ...
            if( idleTaper < configPage2.idleTaperTime )
            {
              uint16_t minValue = table2D_getValue(&iacCrankStepsTable, (toStorageTemperature(currentStatus.coolant))) * 3;
              if( idle_pid_target_value < minValue<<2 ) { idle_pid_target_value = minValue<<2; }
              uint16_t maxValue = idle_pid_target_value>>2;
              if( configPage6.iacAlgorithm == IAC_ALGORITHM_STEP_OLCL ) { maxValue = table2D_getValue(&iacStepTable, (toStorageTemperature(currentStatus.coolant))) * 3; }
            ...
            {
              //Standard running
              FeedForwardTerm = (table2D_getValue(&iacStepTable, (toStorageTemperature(currentStatus.coolant))) * 3)<<2; //All temps are offset by 40 degrees. Step counts are divided by 3 in TS. Multiply back out here
