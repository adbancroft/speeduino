...
static inline void checkAirConCoolantLockout(void)
{
  ...
  // ---------------------------
  int offTemp = toWorkingTemperature(configPage15.airConClTempCut);
...
void fanControl(void)
{
  ...
  {
    int onTemp = toWorkingTemperature(configPage6.fanSP);
  ...
  else if( configPage2.fanEnable == 2 )// PWM Fan control
  {
    ...
    if (fanPermit == true)
      ...
      {
        byte tempFanDuty = table2D_getValue(&fanPWMTable, toStorageTemperature(currentStatus.coolant)); //In normal situation read PWM duty from the table
...
void initialiseAuxPWM(void)
{
  ...
  if( configPage6.vvtEnabled > 0)
  {
    ...
    vvtTimeHold = false;
    if (currentStatus.coolant >= toWorkingTemperature(configPage4.vvtMinClt)) { vvtIsHot = true; } //Checks to see if coolant's already at operating temperature
  }
...
void boostControl(void)
{
  if( configPage6.boostEnabled==1 )
  {
    ...
    {
      //Open loop
      if ( (configPage9.boostByGearEnabled > 0) && isExternalVssMode(configPage2.vssMode) ){ boostByGear(); }
...
{
  if( (configPage6.vvtEnabled == 1) && (currentStatus.coolant >= toWorkingTemperature(configPage4.vvtMinClt)) && (BIT_CHECK(currentStatus.engine, BIT_ENGINE_RUN)))
  {
...
void nitrousControl(void)
{
  ...
  if(configPage10.n2o_enable > 0)
  {
    ...
    if( (isArmed == true) && (currentStatus.coolant > toWorkingTemperature(configPage10.n2o_minCLT)) && (currentStatus.TPS > configPage10.n2o_minTPS) && (currentStatus.O2 < configPage10.n2o_maxAFR) && (currentStatus.MAP < (uint16_t)(configPage10.n2o_maxMAP * 2)) )
    {
...
void wmiControl(void)
{
  ...
  if( (configPage10.vvt2Enabled == 0) && (configPage10.wmiEnabled >= 1) )
  {
    ...
    {
      BIT_CLEAR(currentStatus.status4, BIT_STATUS4_WMI_EMPTY);
      if( (currentStatus.TPS >= configPage10.wmiTPS) && (currentStatus.RPMdiv100 >= configPage10.wmiRPM) && ( (currentStatus.MAP / 2) >= configPage10.wmiMAP) && ( toStorageTemperature(currentStatus.IAT) >= configPage10.wmiIAT) )
      {
