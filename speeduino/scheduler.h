...
struct Schedule {
  ...
  volatile COMPARE_TYPE _nextStartCompare;    ///< Planned start of next schedule (when current schedule is RUNNING)
...
static inline void _setSchedule(Schedule &schedule, uint32_t timeout, uint32_t duration) {
 ...
 * @param openAngle The angle to open the fuel injector
 * @param crankAngle The current crank angle
 */
static inline void setFuelSchedule(FuelSchedule &schedule, uint16_t openAngle, int16_t crankAngle) {
  uint32_t delay = _calculateInjectorTimeout(schedule, openAngle, crankAngle);  
 ...
 * @return uint16_t 
 */
static inline uint16_t calculateInjectorStartAngle(uint16_t pwDegrees, uint16_t tdcOffset, uint16_t injAngle);
