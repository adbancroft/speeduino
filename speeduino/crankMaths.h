...
static constexpr uint32_t MICROS_PER_DEG_1_RPM = (uint32_t)UDIV_ROUND_CLOSEST(MICROS_PER_MIN, 360UL, uint32_t);
 ...
 * 
 * @param revTime The crank revolution time in µS.
 ...
 */
static inline uint32_t getRevolutionTime(void) {
  extern uint32_t revolutionTime;
...
static inline uint16_t rpmFromRevolutionTime(void) {
  if (getRevolutionTime()<=(uint32_t)UINT16_MAX) {
    return udiv_32_16_closest(MICROS_PER_MIN, getRevolutionTime());
  } else {
    return UDIV_ROUND_CLOSEST(MICROS_PER_MIN, getRevolutionTime(), uint32_t);
  }
}
