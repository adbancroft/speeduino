...
void loop(void)
{
    ...
    if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_4HZ))
    {
      ...
      {
        currentStatus.CLIdleTarget = (byte)table2D_getValue(&idleTargetTable, toStorageTemperature(currentStatus.coolant)); //All temps are offset by 40 degrees
    ...
    if ((currentStatus.hasSync || BIT_CHECK(currentStatus.status3, BIT_STATUS3_HALFSYNC)) && (currentStatus.RPM > 0))
    {
      ...
      currentStatus.afrTarget = calculateAfrTarget(currentStatus, configPage2, configPage6);
      ...
      if (BIT_CHECK(currentStatus.spark, BIT_SPARK_HLAUNCH) && (configPage6.lnchHardLim < maxAllowedRPM) ) { maxAllowedRPM = configPage6.lnchHardLim; }
