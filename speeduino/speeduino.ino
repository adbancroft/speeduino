...
static void calculateIgnitionAngles(void);
...
static CRITICAL_INLINE void setIgnitionSchedule(uint8_t index, uint16_t crankAngle, uint16_t totalDwell) {
...
static CRITICAL_INLINE void setIgnitionSchedules(uint16_t crankAngle, uint16_t totalDwell) {
  ...
  for (uint8_t index=1; index<_countof(ignitionSchedules); ++index) {
    setIgnitionSchedule(index, crankAngle, totalDwell);
  }
} 

static CRITICAL_INLINE void setFuelSchedule(uint8_t index, uint16_t crankAngle) {
...
static CRITICAL_INLINE void setFuelSchedules(uint16_t crankAngle) {
  for (uint8_t index=1; index<_countof(fuelSchedules); ++index) {
    setFuelSchedule(index, crankAngle);
  }  
}
 ...
 */
static void calculateIgnitionAngles()
{
  const uint16_t dwellAngle = timeToAngleDegPerMicroSec(currentStatus.dwell); //Convert the dwell time to dwell angle based on the current engine speed
