...
/**
 * @brief Will the result of division fit into TDivisor?
 * 
 * @tparam TDividend Dividend type 
 * @tparam TDivisor Divisor type
...
static constexpr inline bool udivDivResultFitsInDivisor(TDividend dividend, TDivisor divisor) {
...
static inline uint16_t udiv_32_16 (uint32_t dividend, uint16_t divisor)
{
    ...
    if (divisor==0U || !udivDivResultFitsInDivisor(dividend, divisor)) { return UINT16_MAX; }
...
static inline uint8_t udiv_16_8 (uint16_t dividend, uint8_t divisor)
{
    ...
    if (divisor==0U || !udivDivResultFitsInDivisor(dividend, divisor)) { return UINT8_MAX; }
