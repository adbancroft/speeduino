...
#define OFFSET_IGNITION 40U ///< Ignition values from the main spark table are offset 40 degrees downwards to allow for negative spark timing
...
extern struct table2D<uint8_t, uint8_t, 4> taeTable; ///< 4 bin TPS Acceleration Enrichment map (2D)
extern struct table2D<uint8_t, uint8_t, 4> maeTable;
extern struct table2D<uint8_t, uint8_t, 10> WUETable; ///< 10 bin Warm Up Enrichment map (2D)
extern struct table2D<uint8_t, uint8_t, 4> ASETable; ///< 4 bin After Start Enrichment map (2D)
extern struct table2D<uint8_t, uint8_t, 4> ASECountTable; ///< 4 bin After Start duration map (2D)
extern struct table2D<uint8_t, uint8_t, 4> PrimingPulseTable; ///< 4 bin Priming pulsewidth map (2D)
extern struct table2D<uint8_t, uint8_t, 4> crankingEnrichTable; ///< 4 bin cranking Enrichment map (2D)
extern struct table2D<uint8_t, uint8_t, 6> dwellVCorrectionTable; ///< 6 bin dwell voltage correction (2D)
extern struct table2D<uint8_t, uint8_t, 6> injectorVCorrectionTable; ///< 6 bin injector voltage correction (2D)
extern struct table2D<uint8_t, uint16_t, 4> injectorAngleTable; ///< 4 bin injector angle curve (2D)
extern struct table2D<uint8_t, uint8_t, 9> IATDensityCorrectionTable; ///< 9 bin inlet air temperature density correction (2D)
extern struct table2D<uint8_t, uint8_t, 8> baroFuelTable; ///< 8 bin baro correction curve (2D)
extern struct table2D<uint8_t, uint8_t, 6> IATRetardTable; ///< 6 bin ignition adjustment based on inlet air temperature  (2D)
extern struct table2D<uint8_t, uint8_t, 6> idleTargetTable; ///< 10 bin idle target table for idle timing (2D)
extern struct table2D<uint8_t, uint8_t, 10> idleAdvanceTable; ///< 6 bin idle advance adjustment table based on RPM difference  (2D)
extern struct table2D<uint8_t, uint8_t, 6> CLTAdvanceTable; ///< 6 bin ignition adjustment based on coolant temperature  (2D)
extern struct table2D<uint8_t, uint8_t, 8> rotarySplitTable; ///< 8 bin ignition split curve for rotary leading/trailing  (2D)
extern struct table2D<uint8_t, uint8_t, 6> flexFuelTable;  ///< 6 bin flex fuel correction table for fuel adjustments (2D)
extern struct table2D<uint8_t, uint8_t, 6> flexAdvTable;   ///< 6 bin flex fuel correction table for timing advance (2D)
extern struct table2D<uint8_t, int16_t, 6> flexBoostTable; ///< 6 bin flex fuel correction table for boost adjustments (2D)
extern struct table2D<uint8_t, uint8_t, 6> fuelTempTable;  ///< 6 bin flex fuel correction table for fuel adjustments (2D)
extern struct table2D<uint8_t, uint8_t, 6> knockWindowStartTable;
extern struct table2D<uint8_t, uint8_t, 6> knockWindowDurationTable;
extern struct table2D<uint8_t, uint8_t, 4> oilPressureProtectTable;
extern struct table2D<uint8_t, uint8_t, 6> wmiAdvTable; //6 bin wmi correction table for timing advance (2D)
extern struct table2D<uint8_t, uint8_t, 6> coolantProtectTable; //6 bin coolant temperature protection table for engine protection (2D)
extern struct table2D<uint8_t, uint8_t, 4> fanPWMTable;
extern struct table2D<int8_t, uint8_t, 4> rollingCutTable;
...
struct config2 {
  ...
  uint16_t oddfire[3]; ///< The ATDC angle of channels 2-4 for oddfire
...
extern struct table2D<uint16_t, uint16_t, 32> cltCalibrationTable; /**< A 32 bin array containing the coolant temperature sensor calibration values */
extern struct table2D<uint16_t, uint16_t, 32> iatCalibrationTable; /**< A 32 bin array containing the inlet air temperature sensor calibration values */
extern struct table2D<uint16_t, uint8_t, 32> o2CalibrationTable; /**< A 32 bin array containing the O2 sensor calibration values */
