...
static void setFuelChannelAngles(void)
{
  ...
  bool isSequential =    (configPage2.injLayout == INJ_SEQUENTIAL) 
                      ...
                      && (_countof(fuelSchedules)>=configPage2.nCylinders);
  ...
  if (configPage2.injTiming) {
    ...
    if (configPage2.nCylinders==2) {
      spacing = configPage2.engineType == EVEN_FIRE ? 180U : configPage2.oddfire[0];
    } else  if (configPage2.nCylinders==4 || configPage2.nCylinders==6 || configPage2.nCylinders==8) {
      spacing = 720U/configPage2.nCylinders;
    } else {
      spacing = (isSequential ? 720U : 360U)/configPage2.nCylinders;
    }
  }
...
{
  bool sequential4Stroke = (configPage4.sparkMode == IGN_MODE_SEQUENTIAL) && (configPage2.strokes == FOUR_STROKE);
  ...
  {
    // Sequential if flagged and we have at least as many channels as cylinders.
    bool isSequential = sequential4Stroke && (_countof(ignitionSchedules)>=configPage2.nCylinders);

    if (configPage2.nCylinders==4 || configPage2.nCylinders==6 || configPage2.nCylinders==8) {
      maxIgnOutputs = isSequential ? configPage2.nCylinders : configPage2.nCylinders/2;
    } else {
      // 1, 2, 3 & 5 cylinder are essentially sequential if we have enough channels
...
#define FUEL_INTERRUPT(index, avr_vector) \
  ISR((avr_vector)) { \
    onFuelScheduleTimer(fuelSchedules[((uint8_t)(index)-1U)]); \
  }
#else
#define FUEL_INTERRUPT(index, avr_vector) \
...
#define IGNITION_INTERRUPT(index, avr_vector) \
  ISR((avr_vector)) { \
    onIgnitionScheduleTimer(ignitionSchedules[(uint8_t)(index)-1]); \
  }
#else
#define IGNITION_INTERRUPT(index, avr_vector) \
...
static inline bool isAnyIgnScheduleRunning(void) {
  uint8_t index=0U;
  while (index<_countof(ignitionSchedules) && !isRunning(fuelSchedules[index])) {
    ++index;
  }
