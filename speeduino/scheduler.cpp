...
#else
static void resetFuelSchedulers(void)
#endif
{
...
#else
static void resetIgnitionSchedulers(void)
#endif
{
...
static void setFuelChannelAngles(void)
{
  ...
  if (configPage2.injTiming==INJ_TIMING_ALTERNATING) {
    ...
    } else  if (configPage2.nCylinders==4U || configPage2.nCylinders==6U || configPage2.nCylinders==8U) {
      spacing = 720U/configPage2.nCylinders;
    } else {
      spacing = (configPage2.injLayout == INJ_SEQUENTIAL ? 720U : 360U)/configPage2.nCylinders;
    }
  }

  // Adjust angles by # of squirts?
  bool useSquirts = (configPage2.injTiming==INJ_TIMING_ALTERNATING) && (currentStatus.nSquirts > 2U /* Will be 1 in sequential mode */)
...
#define IGNITION_INTERRUPT(index, avr_vector) \
  ISR((avr_vector)) { \
    onIgnitionScheduleTimer(ignitionSchedules[(uint8_t)(index)-1U]); \
  }
#else
#define IGNITION_INTERRUPT(index, avr_vector) \
...
void matchInjectionModeToSyncStatus(void) {
  if (configPage2.injLayout == INJ_SEQUENTIAL) {
    if (currentStatus.hasSync==1) {
      ...
      // Nothing to do but keep the MISRA checked happey
    }
  }
}
