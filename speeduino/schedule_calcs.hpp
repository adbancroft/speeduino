...
static inline uint16_t _updatePwAngleCache(uint16_t pw, injectorAngleCalcCache *pCache) {
  ...
  int16_t pwDelta =  (int16_t)pCache->pw-(int16_t)pw;
  if (abs(pwDelta)>PW_DELTA_THRESHOLD) {
...
static inline void adjustCrankAngle(IgnitionSchedule &schedule, int16_t crankAngle) {
  ...
  ATOMIC() { // Prevent race conditions with the timer interrupt.
    ...
    else if((schedule._status==PENDING) && (currentStatus.startRevolutions > MIN_CYCLES_FOR_CORRECTION) && (schedule.chargeAngle>crankAngle) ) { 
