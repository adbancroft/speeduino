...
static inline uint16_t _adjustToTDC(int16_t angle, uint16_t angleOffset, uint16_t maxAngle) {
...
static inline uint32_t _calculateInjectorTimeout(const FuelSchedule &schedule, int16_t injectorStartAngle, int16_t crankAngle)
{
...
static inline int16_t _calculateCoilChargeAngle(int16_t dwellAngle, int16_t dischargeAngle) {
  if (dischargeAngle>dwellAngle) {
    return dischargeAngle - dwellAngle;
  }
  return dischargeAngle + CRANK_ANGLE_MAX_IGN - dwellAngle;
}
