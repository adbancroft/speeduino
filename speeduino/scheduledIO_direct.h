...
static inline void openInjector_DIRECT(uint8_t channelIndex) { if (channelIndex<=_countof(injectorPins)) { setPin_High(injectorPins[channelIndex-1U]); } }
static inline void closeInjector_DIRECT(uint8_t channelIndex) { if (channelIndex<=_countof(injectorPins)) { setPin_Low(injectorPins[channelIndex-1U]); } }
static inline void toggleInjector_DIRECT(uint8_t channelIndex) { if (channelIndex<=_countof(injectorPins)) { togglePin(injectorPins[channelIndex-1U]); } }

// ======================== Ignition ========================

static inline void coilStartCharging_DIRECT(uint8_t channelIndex) { 
    if (channelIndex<=_countof(ignitionPins)) { 
